...
def _get_repo_nltk_data_dir() -> Path:
    return Path(__file__).resolve().parents[4] / 'nltk_data'


def _configure_nltk_data_path() -> None:
    nltk_data_dir = _get_repo_nltk_data_dir()
    nltk_data_dir_str = str(nltk_data_dir)
    if nltk_data_dir.is_dir() and nltk_data_dir_str not in nltk.data.path:
...
def _ensure_nltk_resource(resource_path: str, download_name: str) -> None:
    ...
    try:
        ...
        nltk.download(download_name, download_dir=str(_get_repo_nltk_data_dir()), quiet=True)
        nltk.data.find(resource_path)
