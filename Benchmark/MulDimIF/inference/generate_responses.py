...
Usage example:
    ...
    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 vllm serve /data/oss_bucket_0/tianze/model/baseline/qwen3_4b_instruct_rubric2token_499/ --port 8001 \
    --served-model-name Qwen/Qwen3-4B-Instruct-2507 --tensor-parallel-size 8
    
    python Code/inference/generate_responses.py \
        --input_data Data/test.json \
        --api_base http://localhost:8000/v1 \
        --model_name Qwen/Qwen3-4B-rubric2token \
        --max_concurrent 128
"""
...
async def process_dataset(
    ...
    async def process_single(prompt_data):
        async with semaphore:
            messages = prompt_data["conversations"]
            try:
            ...
            new_data = prompt_data
            new_data['conversations'] = messages
            return new_data
