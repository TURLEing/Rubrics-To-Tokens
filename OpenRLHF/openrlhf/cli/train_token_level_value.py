...
from openrlhf.trainer.token_level_value_trainer import TokenLevelValueTrainer
...
def train(args):
    ...
    train_dataset = TokenLevelValueDataset(
        train_data,
        tokenizer,
        args.max_len,
        strategy,
        input_template=args.input_template,
        prompt_key=args.prompt_key,  # Deprecated, kept for compatibility
        criteria_key=args.criteria_key,
        response_key=args.response_key,
        token_labels_key=args.token_labels_key,
        char_labels_key=args.char_labels_key,
    ...
    eval_dataset = TokenLevelValueDataset(
        eval_data,
        tokenizer,
        args.max_len,
        strategy,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt_path", type=str, default="./ckpt/checkpoints_token_level_value")
    ...
    # Models
    parser.add_argument("--pretrain", type=str, default=None, required=True)
    ...
    parser.add_argument("--prompt_key", type=str, default="prompt", help="Key for prompt in dataset (deprecated, kept for compatibility)")
    parser.add_argument("--criteria_key", type=str, default="criteria", help="Key for criteria in dataset")
    parser.add_argument("--response_key", type=str, default="response", help="Key for response in dataset")
    parser.add_argument("--token_labels_key", type=str, default="token_labels", help="Key for token-level labels")
    parser.add_argument("--char_labels_key", type=str, default="char_labels", help="Key for character-level labels")
    parser.add_argument(
        "--label_format",
        type=str,
        default="token",
        choices=["token", "char"],
        help="Label format: 'token' for token-level labels, 'char' for character-level labels",
    )
    ...
    parser.add_argument("--wandb_project", type=str, default="openrlhf_train_token_level_value")
    parser.add_argument(
        "--wandb_run_name",
        type=str,
        default="token_level_value_%s" % datetime.now().strftime("%m%dT%H:%M"),
    )

    # TensorBoard parameters
    parser.add_argument("--use_tensorboard", type=str, default=None, help="TensorBoard logging path")

    # Sample printing parameters
    parser.add_argument("--print_samples", action="store_true", default=True, help="Print sample data during training")
    parser.add_argument("--no_print_samples", dest="print_samples", action="store_false", help="Disable printing sample data")
    parser.add_argument("--print_sample_steps", type=int, nargs="+", default=[0, 100, 500, 1000], help="Training steps at which to print samples")
