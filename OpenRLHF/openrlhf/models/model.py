...
def get_llm_for_token_level_value(
    model_name_or_path: str,
    *,
    bf16=True,
    load_in_4bit=False,
    ...
    **kwargs,
    ...
    """Retrieve a transformer model with a token-level value head for binary classification.

    This function loads a pretrained transformer model and attaches a linear layer for
    token-level binary classification (0/1 labels).
    ...
    Args:
        ...
        ds_config (dict, optional): Deepspeed configuration. Defaults to None.
        init_value_head (bool, optional): Initialize the value head. Defaults to False.
        value_head_prefix (str, optional): Prefix for the value head. Defaults to "score".
        device_map (dict, optional): Map of devices for model loading. Defaults to None.
        packing_samples (bool, optional): Whether to pack samples during training. Defaults to False.
    ...
    Returns:
        nn.Module: A pretrained transformer model with a token-level value head.
    """
    config = AutoConfig.from_pretrained(model_name_or_path, trust_remote_code=True)
    config._attn_implementation = attn_implementation
    ...
    base_class = AutoModel._model_mapping[type(config)]
    base_pretrained_class = base_class.__base__
    cls_class = _get_token_level_value_model(base_pretrained_class, base_class, value_head_prefix, packing_samples)
    ...
    # Initialize value_head manually for token-level classification
    if init_value_head:
        value_head = getattr(model, value_head_prefix)
        if dschf is not None:
            logger.info("initialize value_head for ZeRO-3 token-level value model training.")
            with deepspeed.zero.GatheredParameters([value_head.weight], modifier_rank=0):
                if torch.distributed.get_rank() == 0:
                    value_head.weight.data.normal_(mean=0.0, std=1 / (config.hidden_size + 1))
        else:
...
def _get_token_level_value_model(base_pretrained_model, base_llm_model, value_head_prefix="score", packing_samples=False):
    class TokenLevelValueModel(base_pretrained_model):
        supports_gradient_checkpointing = True
        ...
        def __init__(self, config: AutoConfig):
            ...
            self.value_head_prefix = value_head_prefix
            # Output dimension is 1 for binary classification (will use sigmoid)
            setattr(self, value_head_prefix, nn.Linear(config.hidden_size, 1, bias=True))
        ...
        def forward(
            ...
            """
            Forward pass for token-level value model.

            Args:
                input_ids: Input token IDs (batch_size, seq_len)
                attention_mask: Attention mask (batch_size, seq_len)
                response_mask: Mask indicating response tokens (batch_size, seq_len), 1 for response, 0 for prompt
                return_output: Whether to return full model outputs
                ring_attn_group: Ring attention group (for packing)
                packed_seq_lens: Packed sequence lengths

            Returns:
                If return_output=False: logits (batch_size, seq_len) - logits for each token
                If return_output=True: (logits, outputs) tuple
            """
            batch, seqlen = input_ids.size()
            forward_attention_mask = attention_mask
            ...
            logits = value_head(last_hidden_states).squeeze(-1)  # (batch, seq_len)

            if self.packing_samples:
                logits = gather_and_pad_tensor(logits, ring_attn_group, ring_attn_pad_len, indices, batch, seqlen)
