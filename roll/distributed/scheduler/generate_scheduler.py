...
class DynamicSamplingScheduler(Scheduler):
    def __init__(self, pipeline_config=None):
        super().__init__()
        self.pipeline_config = pipeline_config
    ...
    async def get_batch_opt_level_0(self, data: DataProto, batch_size: int) -> DataProto:
        ...
        skip_rewards = data.meta_info.get("skip_rewards", False)
        ...
        while len(completed_data) < batch_size:
            ...
            if skip_rewards:
                from roll.distributed.scheduler.user_defined_rollout_loop import build_skip_reward_output
                batch_rewards = build_skip_reward_output(req=batch)
            else:
                batch_rewards = await self.reward_scheduler.compute_rewards(
                    data=batch, reward_clusters=self.reward_clusters, pipeline_config=self.pipeline_config
                )
