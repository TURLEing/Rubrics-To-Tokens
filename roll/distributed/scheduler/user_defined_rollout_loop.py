...
def build_skip_reward_output(req: DataProto) -> DataProto:
    output_count = req.batch.batch_size[0]
    scores = torch.zeros(output_count, dtype=torch.float16, device=req.batch.device)
    token_level_rewards = torch.zeros_like(req.batch["responses"], dtype=torch.float16)
...
class UserDefinedRolloutLoop:
    ...
    async def _generate_and_reward(
        ...
        for _ in range(5): # limit max retry times, otherwise may cause dead loop
            ...
            if req.meta_info.get("skip_rewards", False):
                rewards = build_skip_reward_output(req=req)
            else:
