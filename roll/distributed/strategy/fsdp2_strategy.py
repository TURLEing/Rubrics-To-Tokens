...
class FSDP2StrategyBase(InferenceStrategy):
    ...
    def save_checkpoint(self, save_dir, global_step, ckpt_id, tag="checkpoint", **kwargs):
        ...
        # In distributed Ray setup, broadcast the save_dir from global rank 0
        if dist.is_initialized():
            save_dir_list = [save_dir]
            dist.broadcast_object_list(save_dir_list, src=0)
            save_dir = save_dir_list[0]
            logger.info(f"Synchronized save_dir across ranks: {save_dir}")
    ...
    def load_checkpoint(self, load_dir, tag="checkpoint", **kwargs):
        ...
        if dist.is_initialized():
            ...
            dist.broadcast_object_list(load_dir_list, src=0)
            load_dir = load_dir_list[0]
            logger.info(f"Synchronized load_dir across ranks: {load_dir}")
        ...
        optimizer = getattr(self, "optimizer", None)
        scheduler = getattr(self, "scheduler", None)
        extra_state_dict = self._load_extra_state_dict(
            base_dir=load_dir,
            world_size=world_size,
            dp_rank=dp_rank,
        )
        ...
        if not used_dcp:
            self._load_checkpoint_from_legacy_shards(
                load_dir=load_dir,
                world_size=world_size,
                dp_rank=dp_rank,
                optimizer=optimizer,
        ...
        if lr_scheduler_state_dict is not None and scheduler is not None:
            scheduler.load_state_dict(lr_scheduler_state_dict)

        if dist.is_initialized():
            dist.barrier()

        logger.info(f"Successfully loaded checkpoint from {load_dir}")
    ...
    def _prepare_fsdp2_model(
        ...
        *,
        ...
        if self.worker_config.model_args.ulysses_size != cp_size:
            # FIXME: ROLL currently do not support ulysses in transformer.version >= 4.53.0
            logger.warning(
...
class FSDP2InferStrategy(FSDP2StrategyBase):
    ...
    def setup_fsdp2_configuration(self):
        ...
        reshard_after_forward = self.worker_config.strategy_args.strategy_config.get("reshard_after_forward", True)
