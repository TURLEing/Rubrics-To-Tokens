...
def prepare_automap_files(model_path: str):
    ...
    for file_name in os.listdir(model_path):
        if file_name.endswith(".py") and os.path.isfile(
...
_LLAMA_FINETUNE_PAD_TOKEN = "<|finetune_right_pad_id|>"


def _set_pad_token(tokenizer) -> None:
    """Set pad_token for tokenizers that don't define one.

    Prefers the dedicated finetune pad token (Llama 3+) over reusing eos_token,
    so that padding and sequence-end signals remain distinct.
    """
    if _LLAMA_FINETUNE_PAD_TOKEN in tokenizer.get_vocab():
        tokenizer.pad_token = _LLAMA_FINETUNE_PAD_TOKEN
    else:
        tokenizer.pad_token = tokenizer.eos_token


def default_tokenizer_provider(
    model_args: "ModelArguments", model_name_or_path: str = None
):
...
def default_processor_provider(
    model_args: "ModelArguments", model_name_or_path: str = None
):
    if model_args.model_type == "diffusion_module":
        return None
...
def load_model(
    ...
    if (
        model_args.attn_implementation is not None
        and model_args.attn_implementation != "auto"
    ):
        setattr(
    ...
    if add_valuehead:
        ...
        model = AutoModelForCausalLMWithValueHead.from_pretrained(
            model, **model_args.model_config_kwargs
        )
        ...
        if vhead_params is not None:
            if is_deepspeed_zero3_enabled():
                ...
                params = [
                    param
                    for _, param in model.v_head.named_parameters(
                        recurse=False
                    )
                ]
                ...
                model.load_state_dict(vhead_params, strict=False)
            logger.info(
                "Loaded valuehead from checkpoint: {}".format(
                    model_name_or_path
                )
            )
...
def patch_model(model, config, use_mcore):
    ...
    if not use_mcore:
        if "qwen2_vl" == model_type or "qwen2_5_vl" == model_type:
            if (
                is_peft_model := getattr(model, "peft_config", None)
                is not None
            ):
            ...
            def _handle_missing_visual(
                self, inputs_embeds: "torch.FloatTensor"
            ):
                ...
                mock_grid_thw = torch.LongTensor([[1, 2, 2]]).to(
                    inputs_embeds.device
                )
                image_embeddings = self.visual(
                    mock_pixel_values, grid_thw=mock_grid_thw
                )
            ...
            def forward_patch(
                ...
                **kwargs,
                ...
                if (
                    kwargs.pop("force_vit_image", False)
                    and pixel_values is None
                ):
                    # force vit forward with mock image to avoid hang
                    inputs_embeds = self.model.embed_tokens(input_ids)
                    inputs_embeds = _handle_missing_visual(
                        self, inputs_embeds
                    )
                if (
                    kwargs.pop("force_vit_video", False)
        ...
        if forward_patch is not None:
            if is_peft_model:
                model.get_base_model().forward = types.MethodType(
                    forward_patch, model.get_base_model()
                )
            else:
...
def default_diffusion_module_provider(
    ...
    if model_args.model_config_kwargs["model_name"] == "wan2_2":
        ...
        raise NotImplementedError(
            f"model_type {model_args.model_type} not implemented yet"
        )
...
def default_actor_model_provider(
    ...
    model_args.model_name_or_path = download_model(
    ...
    if (
        ...
        # megatron
        if (
            model_args.moe_aux_loss_coef is not None
            and training_args.moe_aux_loss_coeff is None
        ):
...
def default_reward_model_provider(
    ...
    if (
        ...
        if model_args.model_type in ["auto_sequence_classification"]:
            logger.info(
                f"use AutoModelForSequenceClassification model {model_args.model_type}"
            )
            config = AutoConfig.from_pretrained(
                model_args.model_name_or_path, trust_remote_code=True
            )
            ...
            logger.info(
                f"use AutoModelForTokenClassification model {model_args.model_type}"
            )
            config = AutoConfig.from_pretrained(
                model_args.model_name_or_path, trust_remote_code=True
            )
            ...
            AutoModelForCausalLMWithValueHead.post_init = (
                no_set_device_hook_post_init
            )
            model = load_model(model_args, is_trainable, True)
            setattr(
                model, "forward", token_classifier_forward.__get__(model)
            )
            setattr(
                model,
                "load_state_dict",
            ...
            logger.info(
                f"patch AutoModelForCausalLMWithValueHead load_state_dict and forward"
            )
        else:
            raise NotImplementedError
        if model.config.pad_token_id is None:
    ...
    if "qwen2" in config.model_type:
        ...
        if is_transformers_version_greater_than("4.52.0"):
            ...
            get_rope_index = types.MethodType(
                Qwen2VLModel.get_rope_index, dummy_self
            )
        else:
