...
from roll.configs.base_rubircs_config import PPOConfig
...
class BenchmarkEvalConfig:
    enabled: bool = field(default=False, metadata={"help": "Whether to enable benchmark evaluation during validation."})
    name: List[Literal["ifeval", "ifbench", "muldimif"]] = field(
        default_factory=lambda: ["ifeval"],
        metadata={"help": "Benchmark evaluation names."},
    )
    input_data: Optional[Union[str, Dict[str, str]]] = field(
        default=None,
        metadata={"help": "Benchmark input jsonl path, or per-benchmark path mapping."},
    )
    output_dir: str = field(default="./output/benchmark_eval", metadata={"help": "Directory to save benchmark outputs."})
    batch_size: int = field(default=32, metadata={"help": "Batch size for benchmark generation."})
    save_outputs: bool = field(default=True, metadata={"help": "Whether to save benchmark responses and eval outputs."})
    max_samples: int = field(default=0, metadata={"help": "Optional cap on the number of benchmark samples. 0 means all."})
    muldimif_max_length: Optional[int] = field(
        default=None,
        metadata={"help": "Override val_prompt_length for MulDimIF only. MulDimIF prompts can be very long (p95 ~5k chars)."},
    )
...
class RLVRConfig(PPOConfig):
    ...
    validation_benchmark: BenchmarkEvalConfig = field(
        default_factory=BenchmarkEvalConfig,
        metadata={"help": "Benchmark evaluation configuration for validation."},
    )
    ...
    # all-or-nothing 只在 rubric 全对时才会赋奖励，更robust不易被hack
    reward_union_type: Literal["mean", "all-or-nothing", "weighted-mean"] = field(
        default="mean",
        metadata={"help": "The type of reward union."}
    )

    # 在 rubric-to-token GRPO 中，Group 的粒度是什么
    rubric_norm_type: Literal[
        "rubric_token_mean-groups_adv",
        "group_token_adv-rubrics_mean",
        "token_only_adv-rubrics_mean",
        "group_adv-token_softmax-rubrics-sum"
    ] = field(default="rubric_token_mean-groups_adv", metadata={"help": "The type of the rubric normalization."})
    rubric_token_level_reward_method: Literal["mix", "add"] = field(default="mix", metadata={"help": "The method to combine rubric token level rewards with advantages: 'mix' for weighted average, 'add' for addition."})
    rubric_token_level_reward_weight: float = field(default=1.0, metadata={"help": "The weight of the rubric token level reward."})
    ...
    def __post_init__(self):
        ...
        if self.discriminator.worker_cls is None:
            self.discriminator.worker_cls = "roll.pipeline.rlvr.rewards.discriminator_worker.DiscriminatorWorker"
