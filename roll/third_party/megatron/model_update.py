...
def gather_and_convert_weights(
    ...
    if model_converter.mca_config.hf_model_type == "qwen3_vl_moe" and ep_group is not None:
        ...
        for handle, (mcore_name, weights) in zip(handles, gathered_named_weights):
            ...
            for ep_rank, weight in enumerate(weights):
                global_moe_index = model_converter.dist_converter.num_layers_for_expert * ep_rank + int(local_moe_index)
                name = mcore_name[: -len(local_moe_index)] + str(global_moe_index)
                converted_weights = (
                    model_converter.convert_to_hf(
                        {name: [weight]}, layer_index_preprocessed=True, moe_index_preprocessed=True
                    )
                    or {}
                )
    ...
    for handle, (mcore_name, weights) in zip(handles, gathered_named_weights):
        if handle is not None:
            handle.wait()
        converted_weights = model_converter.convert_to_hf({mcore_name: weights}, layer_index_preprocessed=True) or {}
        hf_named_weights.extend([(name, weight) for name, weight in converted_weights.items()])
...
def _gather_hf_weights(
    model_converter: ModelConverter, named_weights: list[tuple[str, torch.Tensor]], buffer_size: Optional[int] = None
):
...
def gather_pp_stage_hf_weights(models: list[McaGPTModel], buffer_size):
...
def gather_weights_meta_cross_pp(models: list[McaGPTModel]):
    ...
    for mcore_name, weight in _iter_vp_stage_named_weights(models, model_converter):
        ...
        named_weights_meta.append(
            {"name": mcore_name, "shape": weight.shape, "dtype": weight.dtype, "pp_stage": pp_rank, "size": weight_size}
        )
    ...
    all_named_weights_meta = sorted([meta for metas in all_named_weights_meta for meta in metas], key=lambda x: x["name"])
    expert_weights_meta = []
    other_weights_meta = []
...
def gather_all_hf_weights(models: list[McaGPTModel], buffer_size: int, weights_meta: Optional[list[dict]]):
    ...
    if pp_size <= 1:
        yield from gather_pp_stage_hf_weights(models, buffer_size)
        return
    ...
    def _gather_batch_params(named_weights_with_stage: list[tuple[str, torch.Tensor, int]]):
        ...
        for mcore_name, weight, pp_stage in named_weights_with_stage:
            ...
            handles.append(
                dist.broadcast(weight, group=mpu.get_pipeline_model_parallel_group(), async_op=True, group_src=pp_stage)
            )
        for handle in handles:
            handle.wait()
        yield from _gather_hf_weights(model_converter, named_weights)
