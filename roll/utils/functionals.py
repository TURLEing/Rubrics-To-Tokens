...
### rubric_token_mean-groups_adv, group_token_adv-rubrics_mean, token_only_adv-rubrics_mean, group_adv-token_softmax-rubrics-sum
# rubric_token_mean-groups_adv: 先对所有rubric的token_level_rewards求均值，然后对group的token_level_rewards 归一化求优势
# group_token_adv-rubrics_mean: 先对于一个rubric的整个group进行归一化求优势，最后对所有rubric的token_level_rewards 求均值
# token_only_adv-rubrics_mean: 在group的单条response序列上对每个rubric上归一化求优势，最后对所有rubric的 token_level_rewards 求均值
# group_adv-token_softmax-rubrics-sum: 在 response-level 上归一化（同GRPO），再通过softmax扩大相关度差异，摊到token上

def reward_norm_rubrics(rubric2token_weights, rubric2token_scores, n_sample=-1, norm_type="rubric_token_mean-groups_adv", response_mask=None):
    if norm_type == "rubric_token_mean-groups_adv":
        token_level_rewards = []
        logger.info(f"len(rubric2token_score): {len(rubric2token_weights)}")
        for rubric2token_weight, rubric2token_score in zip(rubric2token_weights, rubric2token_scores): #（512, rubcrics, length)
            # logger.info(torch.tensor(rubric2token_score).size()) # torch.Size([3368, 6144])
            rubric2token_score = torch.tensor(rubric2token_score, dtype=float).unsqueeze(-1) * torch.tensor(rubric2token_weight, dtype=float)
            token_level_rewards.append(torch.tensor(rubric2token_score, dtype=float).mean(dim=0)) # [rubircs_num, seq_length] --> [seq_length]
        token_level_rewards = torch.stack(token_level_rewards, dim=0)
        logger.info(f"token_level_rewards shape: {token_level_rewards.size()}")
        reshape_reward = token_level_rewards.reshape(n_sample, -1, token_level_rewards.size()[-1]) # group_size, group_num, seq_len
        logger.info(f"reshape_reward shape: {reshape_reward.size()}")
        # e.g. [512, 4096] -> [8, 64, 4096]
        # 8 = n_sample (每个 prompt 的 response 数)

        rewards = reshape_reward
        if response_mask is not None:
            reshape_mask = response_mask.float().reshape(n_sample, -1, response_mask.size(-1))
            assert reshape_mask.shape == reshape_reward.shape, (
                f"reshape_mask {reshape_mask.shape} != reshape_reward {reshape_reward.shape}"
            )
            valid_count = reshape_mask.sum(dim=(1, 2), keepdim=True).clamp(min=1)
            reward_mean = (reshape_reward * reshape_mask).sum(dim=(1, 2), keepdim=True) / valid_count
            reward_std = (((reshape_reward - reward_mean) ** 2 * reshape_mask).sum(dim=(1, 2), keepdim=True) / valid_count).sqrt()
        else:
            reward_mean = reshape_reward.mean(dim=(1, 2), keepdim=True)
            reward_std = torch.std(reshape_reward, dim=(1, 2), keepdim=True)
        logger.info(f"reward_mean shape: {reward_mean.size()}")
        logger.info(f"reward_std  shape: {reward_std.size()}")
        normalized_rewards = (rewards - reward_mean) / (reward_std + 1e-6)
        if response_mask is not None:
            normalized_rewards = normalized_rewards * reshape_mask
        normalized_rewards = normalized_rewards.reshape(*token_level_rewards.size())
        logger.info(f"normalized_rewards shape: {normalized_rewards.size()}")

    elif norm_type == "group_token_adv-rubrics_mean":
        normalized_rewards = []
        batch_size = len(rubric2token_weights)
        # [64 * 8, rubric_num(3), seq_len(4096)] —- group --> [8, 3, 4096] -- reshape --> [3, 8, 4096] 
        # -- group by all --> [3, 1, 1] mean/std -- normalize --> [3, 8, 4096] -- mean by rubrics --> [8, 4096]
        for start_idx in range(0, batch_size, n_sample): 
            group_rubric2token_scores = torch.tensor(rubric2token_weights[start_idx:start_idx+n_sample], dtype=float) * torch.tensor(rubric2token_scores[start_idx:start_idx+n_sample], dtype=float).unsqueeze(-1)
            group_size, rubrics_size, seq_len = group_rubric2token_scores.size()
            group_rubric2token_scores = group_rubric2token_scores.reshape(rubrics_size, group_size, seq_len) # (rubrics_num, group_size, seq_len)
            if response_mask is not None:
                group_mask = response_mask[start_idx:start_idx+n_sample].float()  # (group_size, seq_len)
                group_mask_expanded = group_mask.unsqueeze(0).expand(rubrics_size, -1, -1)  # (rubrics_num, group_size, seq_len)
                assert group_mask_expanded.shape == group_rubric2token_scores.shape, (
                    f"group_mask_expanded {group_mask_expanded.shape} != group_rubric2token_scores {group_rubric2token_scores.shape}"
                )
                valid_count = group_mask_expanded.sum(dim=(1, 2), keepdim=True).clamp(min=1)  # (rubrics_num, 1, 1)
                rubrics_mean = (group_rubric2token_scores * group_mask_expanded).sum(dim=(1, 2), keepdim=True) / valid_count
                rubrics_std = (((group_rubric2token_scores - rubrics_mean) ** 2 * group_mask_expanded).sum(dim=(1, 2), keepdim=True) / valid_count).sqrt()
            else:
                rubrics_mean = group_rubric2token_scores.mean(dim=(1,2), keepdim=True)  # (rubrics_num, 1, 1)
                rubrics_std = torch.std(group_rubric2token_scores, dim=(1,2), keepdim=True)  # (rubrics_num, 1, 1)
            normalized_group = (group_rubric2token_scores - rubrics_mean) / (rubrics_std + 1e-6)  # (rubrics_num, group_size, seq_len)
            if response_mask is not None:
                normalized_group = normalized_group * group_mask_expanded
            group_rewards = normalized_group.mean(dim=0)  # (group_size, seq_len)
            normalized_rewards.append(group_rewards)
        normalized_rewards = torch.stack(normalized_rewards, dim=0)  # (64, 8, seq_len)
        normalized_rewards = normalized_rewards.reshape(batch_size, -1) # (512, seq_len)
        logger.info(f"normalized_rewards: {normalized_rewards.size()}") # [512, 4096] + token_level_reard
    elif norm_type == "token_only_adv-rubrics_mean":
        normalized_rewards = []
        batch_size = len(rubric2token_weights)
        # [64 * 8, rubric_num(3), seq_len(4096)] —- group --> [8, 3, 4096] -- reshape --> [3, 8, 4096] 
        # -- group by seq --> [3, 8, 1] mean/std -- normalize --> [3, 8, 4096] -- mean by rubrics --> [8, 4096]
        for start_idx in range(0, batch_size, n_sample):
            group_rubric2token_scores = torch.tensor(rubric2token_weights[start_idx:start_idx+n_sample], dtype=float) * torch.tensor(rubric2token_scores[start_idx:start_idx+n_sample], dtype=float).unsqueeze(-1)
            ...
            if response_mask is not None:
                ...
                valid_count = group_mask_expanded.sum(dim=2, keepdim=True).clamp(min=1)  # (rubrics_num, group_size, 1)
                rubrics_seq_mean = (group_rubric2token_scores * group_mask_expanded).sum(dim=2, keepdim=True) / valid_count
                rubrics_seq_std = (((group_rubric2token_scores - rubrics_seq_mean) ** 2 * group_mask_expanded).sum(dim=2, keepdim=True) / valid_count).sqrt()
            else:
                rubrics_seq_mean = group_rubric2token_scores.mean(dim=2, keepdim=True)  # (rubrics_num, group_size, 1)
                rubrics_seq_std = torch.std(group_rubric2token_scores, dim=2, keepdim=True)  # (rubrics_num, group_size, 1)
            normalized_group = (group_rubric2token_scores - rubrics_seq_mean) / (rubrics_seq_std + 1e-6)  # (rubrics_num, group_size, seq_len)
            if response_mask is not None:
                normalized_group = normalized_group * group_mask_expanded
            group_rewards = normalized_group.mean(dim=0)  # (group_size, seq_len)
            normalized_rewards.append(group_rewards)
        normalized_rewards = torch.stack(normalized_rewards, dim=0)  # (64, 8, seq_len)
        normalized_rewards = normalized_rewards.reshape(batch_size, -1)  # (512, seq_len)
        logger.info(f"normalized_rewards: {normalized_rewards.size()}")  # [512, 4096]
    elif norm_type == "group_adv-token_softmax-rubrics-sum":
        normalized_rewards = []
        batch_size = len(rubric2token_scores)
        # [64 * 8, rubric_num(3), seq_len(4096)] —- group --> [8, 3, 4096]
        # 1. softmax(weights, dim=1) -> [group_size, rubric_num, seq_len] 每个token对每个rubric的重要性占比
        # 2. rubric advantage: (scores - mean) / std -> [group_size, rubric_num]
        # 3. softmax_weights * advantage.unsqueeze(-1) -> [group_size, rubric_num, seq_len]
        # 4. sum over rubrics -> [group_size, seq_len]
        for start_idx in range(0, batch_size, n_sample):
            group_rubric2token_weights = torch.tensor(rubric2token_weights[start_idx:start_idx+n_sample], dtype=torch.float32)  # [group_size, rubric_num, seq_len]
            group_rubric2token_scores = torch.tensor(rubric2token_scores[start_idx:start_idx+n_sample], dtype=torch.float32)  # [group_size, rubric_num]
            
            # Step 1: 对每个token计算softmax，得到该token对每个rubric的重要性占比
            # softmax along rubric dimension (dim=1)
            token_rubric_importance = torch.softmax(group_rubric2token_weights, dim=1)  # [group_size, rubric_num, seq_len]
            
            # Step 2: 计算rubric-level的advantage (在group内进行normalize)
            rubric_mean = group_rubric2token_scores.mean(dim=0, keepdim=True)  # [1, rubric_num]
            rubric_std = group_rubric2token_scores.std(dim=0, keepdim=True)  # [1, rubric_num]
            rubric_advantage = (group_rubric2token_scores - rubric_mean) / (rubric_std + 1e-6)  # [group_size, rubric_num]
            
            # Step 3: 将rubric advantage乘以token的重要性占比
            # rubric_advantage: [group_size, rubric_num] -> [group_size, rubric_num, 1]
            weighted_advantage = token_rubric_importance * rubric_advantage.unsqueeze(-1)  # [group_size, rubric_num, seq_len]
            
            # Step 4: 按softmax权重加权求和作为该token的最终advantage
            token_advantage = weighted_advantage.sum(dim=1)  # [group_size, seq_len]
            
            normalized_rewards.append(token_advantage)
        normalized_rewards = torch.stack(normalized_rewards, dim=0)  # (num_groups, group_size, seq_len)
        normalized_rewards = normalized_rewards.reshape(batch_size, -1)  # (batch_size, seq_len)
        logger.info(f"normalized_rewards: {normalized_rewards.size()}")  # [512, 4096]

    return normalized_rewards

def reward_norm_rubrics_for_p(rubric2token_weight, score_groups, n_sample=-1, norm_type="mean"):
    normalized_rewards = []
    batch_size = len(rubric2token_weight)
    # bs, rubric_num
    for start_idx in range(0, batch_size, n_sample): # [32 * 8, x?] —> [8, x!] -> [8, 3, 4096] -> [3, 8, 4096] -> [3, 1, 1] mean [3, 1, 1] std --> [3, 8, 4096]--> [8, 4096]
        rubric2token_weight_group = torch.stack(rubric2token_weight[start_idx : start_idx + n_sample], dim=0).float() # [group_size, rubric_num, seq_len]
        score_group = score_groups[start_idx : start_idx + n_sample] # [group_size, rubric_num]
        score_group = torch.tensor(score_group, dtype=torch.float32)
        normalized_rewards_group = (score_group - score_group.mean(dim=0, keepdim=True)) / (score_group.std(dim=0, keepdim=True) + 1e-6)
        rubric2token_advantage_group = normalized_rewards_group.unsqueeze(-1) * rubric2token_weight_group
        rubric2token_advantage_group = rubric2token_advantage_group.sum(dim=1)  # (group_size, seq_len)
        normalized_rewards.append(rubric2token_advantage_group)
    normalized_rewards = torch.stack(normalized_rewards, dim=0)  # (group_size, seq_len)
    normalized_rewards = normalized_rewards.reshape(batch_size, -1)
    logger.info(f"normalized_rewards: {normalized_rewards.size()}") # [512, 4096] + token_level_reard
    return normalized_rewards

@torch.no_grad()
def reward_postprocess_rubrics(
    data: "DataProto",
    rubric2token_weight,
    rubric2token_score,
    ...
    # 对reward进行处理: 可以灵活定义不同的normalization方法
    rubcric_token_level_rewards = reward_norm_rubrics(
                    rubric2token_weight,
                    rubric2token_score, 
                    n_sample=pipeline_config.actor_infer.generating_args.num_return_sequences,
                    norm_type=pipeline_config.rubric_norm_type,
                    response_mask=response_mask
                    )

    return rubcric_token_level_rewards[:, 1:]


@torch.no_grad()
def reward_postprocess_rubrics_for_p(data: "DataProto", rubric2token_scores, score_groups, pipeline_config: RLVRConfig, running_ctrl):
    response_level_metrics = {"critic/reward_clip_frac": 0.0}
    # 对reward进行处理: 可以灵活定义不同的normalization方法
    rubcric_token_level_rewards = reward_norm_rubrics_for_p(
                    rubric2token_scores, 
                    score_groups,
                    n_sample=pipeline_config.actor_infer.generating_args.num_return_sequences,
                    norm_type=pipeline_config.rubric_norm_type
...
def compute_advantage_p(
    ...
    whiten_rewards=False,
    response_mask=None,
    rubcric_token_level_rewards=None,
    reward_method="mix",
    reward_weight=0.5
):
    if response_mask is None:
    ...
    if adv_estimator == "gae":
        ...
        logger.info(f"rubcric_token_level_rewards: {rubcric_token_level_rewards.size()}")
        logger.info(f"advantages: {advantages.size()}")
        if reward_method == "mix":
            advantages = (1-reward_weight) * advantages + reward_weight * rubcric_token_level_rewards
            returns = (1-reward_weight) * returns + reward_weight * rubcric_token_level_rewards
        elif reward_method == "add":
            advantages = advantages + reward_weight * rubcric_token_level_rewards
            returns = returns + reward_weight * rubcric_token_level_rewards
        else:
            raise NotImplementedError
    else:
        raise NotImplementedError
